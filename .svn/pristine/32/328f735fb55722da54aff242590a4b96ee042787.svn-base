  ...
  <body>
	<div style="position: absolute;top:10px;left:10px;right:10px;bottom: 10px;">
		<form  class="layui-form">
			<div class="layui-form-item">
			    ...
			    <div class="layui-input-block">
			      <input type="text" name="name" value="${task.name }"required  lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input">
			    </div>
			</div>		
			<div class="layui-form-item">
			...
			<div class="layui-form-item">
			    ...
			    <div class="layui-input-block">
			      	<textarea name="description"  required  placeholder="请输入项目描述" class="layui-textarea">${task.description }</textarea>
			    </div>
			</div>
			<div class="layui-form-item">
				...
				<div class="layui-inline">
					...
					<div class="layui-input-inline">
						<input type="text" name="startTime" required lay-verify="required"
							autocomplete="off" value="${task.startTime }"
							class="layui-input"
					...
					<div class="layui-input-inline">
						<input type="text" name="endTime" required lay-verify="required"
							autocomplete="off" class="layui-input" value="${task.endTime }"
							onclick="layui.laydate({elem: this, istime: true, format: 'YYYY-MM-DD'})">
					</div>
				</div>
			</div>
			<input name="teamId" type="hidden" value="${task.teamId }">
			<input name="createPeople" type="hidden" value="${task.createPeople }">
			<input name="id" type="hidden" value="${task.id }">
			<input name="project" type="hidden" value="${task.project }">
			<input name="groupId" type="hidden" value="${task.groupId }">
			<input name="sort" type="hidden" value="${task.sort }">
			<input type="hidden" name = "topTeamId" value="${teamId }">
			<input type="hidden" name = "operationBy" value="${userId }">
			<button class="layui-btn" lay-submit lay-filter="modifyTask" style="float: right;margin-right: 20px;">修改</button>
		</form>
	</div>
	  ...
	  layui.use(['element','layer','form','laydate','upload'], function(){
			...
			//监听提交按钮
			form.on('submit(modifyTask)',function(data){
				//获取参与人员
				var members = [];
