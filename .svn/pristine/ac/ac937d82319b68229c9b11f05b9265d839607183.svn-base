	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.resultType=="0"){
				...
				window.location.href="redirectToLogin.action"; 
			}else{
		...
		<div class="row">
			...
			<div id="mainContainer" class="col-md-4">
				<form id="takeBackPsdFrom" name="takeBackPsdFrom" class="layui-form">
					...
					<bottun type="button" id="btn_modifyPsd"  lay-submit="" lay-filter="getBackPWD" class="layui-btn btn_submit  layui-btn-disabled" >找回密码</bottun>
				</form>
			</div>
		</div>
	</div>
